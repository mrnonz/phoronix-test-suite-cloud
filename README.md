...
docker-compose exec pts /phoronix-test-suite/phoronix-test-suite result-file-to-csv "my_test_results"
```

The argument to `result-file-to-csv` must match the saved results identifier. By default the compose file sets `TEST_RESULTS_NAME` to `my_test_results`; override it (for example in a `.env` file or `docker-compose exec -e TEST_RESULTS_NAME=my_test_results ...`) so it matches the name you pass to `result-file-to-csv`.
